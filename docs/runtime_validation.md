...
Prestige is a required standalone custom mod rather than part of `bcfixes`. Server bundles make `run.sh` the prestige-supervised default entrypoint. The permission-level-4 escape-hatch flow is `/prestige stage`, followed by `/prestige commit <world-name>`; `/prestige cancel` removes a staged request. `/prestige gui [reset|schematics]` opens the client menu. Remote menus cannot satisfy the formed-and-attuned physical condenser check for destructive actions. `run-forge.sh` remains the explicit diagnostic bypass and never performs regeneration.

The stable V4 surface contains only the implemented reset and schematic-library behavior. An operator selects an allowlisted biome, stages it, confirms the exact world name, and commits. The generation-bound supervisor archives the complete world, generates a random-seed successor, checks exact biome and fresh player state, retries three times, and restores the old world on failure. Perks, unspent points, ballots, season/hour/onboarding votes, and seed proposals are not exposed.

Traces is also a required standalone custom mod. Its shards live under `world/data/traces/<namespace>/<dimension>/`, so Prestige archives and restores them with the owning world. The V4 wrapper fixture proves legacy-lineage rejection, exact archive verification, old-world Traces inclusion, successor isolation, distinct random retry seeds, console forwarding, crash-safe committed and rolled-back cleanup, repeated resets in one supervisor lifetime, interruption recovery, idempotent lineage award, and automatic rollback after three bad health reports. Terminal cleanup retains the reset request as its recovery marker until all other cleanup is durable. A fixed PID/start-time contract prevents an orphaned successor from overlapping a replacement Forge process. Legacy server-root `data/traces` is deliberately neither imported nor deleted.

Operators can verify an archive without creating or changing Prestige state with `./prestige-server.sh verify-archive ARCHIVE LINEAGE_ID TRANSACTION_ID`. Schematic publication limits compressed uploads to 256 KiB and parsed NBT to 64 MiB. The V2 catalog removes entities and rebuilds block-entity payloads only through Create 6.0.8's explicit safe-NBT writers before content addressing; unsupported NBT is discarded, while Schematicannon remains responsible for material accounting.
