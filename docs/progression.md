...
The first Create era is a manual workshop, not passive automation. The exact hand-crafting allowlist is recorded in `kubejs/config/create_tcon_bootstrap.json`: the crank, basic transmission, and bounded starter workstations can be assembled by hand, but the Deployer and both andesite casings cannot. A hand-cranked press and basin compact the first Deployer from seared/TCon-era parts; that Deployer earns ordinary andesite casing, sequenced andesite machine casing, and the `kubejs:workshop_governor`. Water wheels, large water wheels, windmill bearings, steam engines, compressed-air engines, diesel engines, and Power Grid motors are then mechanical-crafting outputs that consume a governor. The creative motor remains recipe-less. This makes the buffed hand crank useful enough to run an actual workshop while ensuring every positive-SU source after it proves Create machinery first.
...
- The hand-cranked press/basin compacts the first `create:deployer`; the Deployer then gates `create:andesite_casing`, sequenced andesite machine casing, and the workshop governor without a casing cycle.
...
- The hand crank is the only manually crafted positive-SU source. Every survival-obtainable passive, combustion, pneumatic, diesel, or electric Create-network source is mechanically crafted behind the machine-earned workshop governor; the maintained registry inventory fails closed when a new source family appears.
...
- Do not allow water, wind, steam, pneumatic, diesel, or electric SU before the Deployer, machine casing, and governor route is complete.
...
Ore acquisition has four complementary sources. Halved Realistic Ores cave scatter keeps ordinary exploratory mining productive; common, much larger ADLODS fields supply settlement-scale bulk matter; strategic deposits control fissiles, oil, and lava-diving materials; and chemistry converts associated byproducts or substitutes between otherwise awkward material families. ADLODS fields use composite Realistic Ores blocks rather than generic single-metal ore, and their dedicated `surface_sample_*` blocks form conspicuous rubble circles above the field without placing non-block crushed ingredients. Their low, separated chip models crop ore-specific texture patches so they read as loose fragments instead of clipped miniature ore blocks; oil seeps use a distinct scatter of thin, dark oil-bearing shale. Collected mineral samples can be milled for a 25% chance of the matching crushed deposit. Every marker also has a deliberately marginal furnace route—normally two associated nuggets or a similarly small resource, including carbon from oil-bearing shale—so exploration debris is useful without turning a surface marker circle into a full ore body. A sample identifies the deposit family but does not reveal its exact shape or depth; effective placement remains a worldgen-verification concern.

The starter catalogue in `global.BC_STARTER_DEPOSITS` currently names coal measures, ironstone, copper sulfide, tin, zinc, lead-zinc vein, quartz vein, and bauxite laterite. The six direct metal deposits use the strict four-nugget/90 mB/270 mB-plus-byproduct contract; coal and quartz remain nonmetal exceptions with their own material-specific recovery. Foundry/byproduct work is therefore a real interpretation upgrade, while Create/PNCR chemistry supplies later material identity rather than replacing TCon's first metallurgy lesson. Gold is an enrichment of copper fields, silver is an enrichment of lead fields, cobalt and the platinum-group metals are enrichments of nickel sulfide fields, and gem varieties are enrichments of corundum fields. Their extremely rare standalone ADLODS entries remain only as a compatibility fallback; the intended search is for the parent geology, with chemistry and synthesis supplying interchangeability where recipes allow it. Excavated gravel variants are part of the deposit surface, not a separate ore family: Realistic Ores stone configured features include gravel replacement targets, `excavated_variants:gravel_*` blocks stay shovel-mineable, and gravel substrate support lives in `defaultresources/excavated_variants`.
...
The first chemistry proof is two bounded loops rather than the full element catalogue. Washed carbon and sulfur plus fermented `#forge:ethanol` make pressure seals for airtight infrastructure; direct sugar synthesis costs extra and exists only as recovery. Washed silica and alumina plus milled calcium carbonate improve grout and seared-brick throughput, while netherrack remains mandatory in the grout batch. Salt, phosphate, cellulose, loose gases, and unrelated pure elements do not gain new early routes in this pass.
