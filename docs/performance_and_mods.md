...
- Dimension Font placement is deployed from the reobfuscated chunk-clipped structure writer so structure callbacks do not write into far chunks under C2ME. Terrain reads no longer misuse `WorldGenLevel.ensureCanWrite` as a preflight check; that call emitted false `setBlock` errors at the world's minimum-height sentinel even when no write followed. Latent chemical clouds are non-targetable hand/raytrace volumes and render as sparse non-occluding particles instead of depth-writing block models.
