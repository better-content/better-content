...
The headful lane remains failed and blocks a playable-stability claim. Source inspection pinned Eureka's menu geometry and packet path plus VS assembly, seamless chunk, Sodium renderer, and teleport behavior in `tools/vs_ships_client_contract.properties` and the harness. The fixture contains four unique blocks; Eureka nondeterministically logs `5..8` BFS entries because its initial neighbor stack can process connected positions twice. A valid DH-enabled repro and a corrected DH-off A/B both assemble one ship at the source transform, then show no geometry. Immediate freeze rules out falling; reconnect creates the client ship and dispatches deferred shipyard chunks; command teleport rules out the original transform. A copied-runtime variant without DH, Embeddium, Oculus, Sodium Options API, or Sodium Dynamic Lights still fails, as does enabling VS connectivity. Classifier remains `assembled_render_failure`; investigate loaded ship chunk population/culling next. Mount, movement, save/reload, two-client sync, Clockwork, Trackwork, and broad pack tests remain gated.
