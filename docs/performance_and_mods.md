...
The source-guided Xvfb lane now drives Eureka's exact 176x166 helm menu and Assemble packet, rebuilds the fixture after onboarding, recovers dead reusable pilots, and requires one registered VS ship. Eureka's BFS log is accepted as `5..8` for the four unique fixture blocks because its initial neighbor stack is not marked visited and can process connected initial neighbors twice. The current client and DH-off A/B both render the fixture and menu, then render no ship geometry. Reconnect loads the client ship object and dispatches all deferred shipyard chunks, while a command teleport still leaves the ship invisible. Removing DH, Embeddium, Oculus, Sodium Options API, and Sodium Dynamic Lights and enabling VS connectivity do not restore rendering. The remaining target is loaded ship chunk population/culling below those renderer/config layers. Movement, mount/camera, save/reload, observer sync, Clockwork, and Trackwork remain gated. Do not claim playable VS stability yet.
