...
fun usage(message: String? = null): Nothing {
    ...
    System.err.println("Usage: tools/btm test scenario-headful vs_ships_release [--bootstrap-mode always|once|never] [--run-root PATH] [--keep-going] [--keep-runs]")
    exitProcess(2)
}
...
specifications += "server_lifecycle" to listOf("test", "scenario", "vs_ships_stability", "--profile", "release", "--cycles", "1")
specifications += "isolation_matrix" to listOf("test", "scenario", "vs_ships_matrix", "--profile", "release", "--cycles", "1", "--keep-going")
val fixtures = listOf("core", "clockwork", "trackwork", "combined")
fixtures.forEachIndexed { fixtureIndex, fixture ->
    specifications += "client_${fixture}_current" to listOf("test", "scenario-headful", "vs_ships_client", "--profile", "release", "--fixture", fixture, "--variant", "current", "--port", (25570 + fixtureIndex).toString())
}
listOf("dh_disabled", "c2me_disabled", "dh_c2me_disabled").forEachIndexed { variantIndex, variant ->
    specifications += "client_combined_$variant" to listOf("test", "scenario-headful", "vs_ships_client", "--profile", "release", "--fixture", "combined", "--variant", variant, "--port", (25580 + variantIndex).toString())
}
