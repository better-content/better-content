...
Server bundles make `run.sh` the prestige-supervised default entrypoint. The permission-level-4 Forge escape-hatch flow is `/prestige stage`, followed by `/prestige commit <world-name>`; `/prestige cancel` removes a staged request. `/prestige gui [reset|perks|schematics]` opens the same client menu for the executing player, while `/prestige gui <player> <tab>` lets an operator or console drive a named online client for screenshot validation. Remote menus cannot satisfy the formed-and-attuned physical condenser check for destructive actions. The supervisor alone consumes a committed request after Minecraft has finalized and released the save. `run-forge.sh` is the explicit raw Forge diagnostic bypass and never performs regeneration.

The first twelve-node perk pass is persisted outside the world save. Generation-scoped ballots rank prerequisite-safe nodes, setting votes, and seed proposals; staging snapshots projected winners into `staged-perks-v1.tsv`, and the coordinator promotes those winners only after a subsequent wrapper restart observes the incremented healthy lineage. The current v2 wrapper still owns biome/seed regeneration; successor season/hour/class-selector profile application remains a follow-up versioned contract rather than an unverified claim.

On 2026-08-05 the exact packaged jar passed the ordinary fresh server/client smoke, then a disposable dedicated server proved the command-driven lifecycle through the real wrapper. `/prestige stage minecraft:plains` wrote the current lineage and world identity; `/prestige commit world` finalized and stopped Forge; the wrapper canonically archived the old world and regenerated it with random seed `-6646293182136085069`. The successor published exact requested/actual `minecraft:plains` health with `fresh_players=true`, the wrapper advanced lineage from zero to one only after the stability window, and no active control request remained. A clean wrapper restart reopened the successor and `/prestige status` returned generation/total/unspent `1/1/1`, proving that the transaction was not replayed. After making `run.sh` the public supervised entrypoint, a second fresh packaged smoke passed with the live process chain `run.sh` to `prestige-server.sh` to `run-forge.sh`. The wrapper fixture separately proves successor-console forwarding, canonical archive verification, distinct random retry seeds, interruption recovery, idempotent award, and automatic old-world restoration after three bad health reports. This is dedicated-server evidence; the final real-client World Condenser button invocation and singleplayer orchestration remain unproved.
...
For the current full-registry judgment pass, `tools/bc audit prepare` is the canonical intake step and packet files—not the generated monolithic ledger—are authored authority. `audit status` is read-only and reports intake readiness separately from judgment completion. A draft packet may preserve historical prose as context but cannot project a verdict; an adjudicated packet must use a different author and adjudicator, cover every member explicitly, and attach runtime evidence to any behavioral or mixed phase marked proven. Historical packets bound to older snapshots never become current merely because their IDs still exist.
