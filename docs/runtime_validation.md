...
The stable V3 surface contains only the implemented reset and schematic-library behavior. An operator selects an allowlisted biome, stages it, confirms the exact world name, and commits. The supervisor archives the complete world, generates a random-seed successor, checks exact biome and fresh player state, retries three times, and restores the old world on failure. Perks, unspent points, ballots, season/hour/onboarding votes, and seed proposals are not exposed.

Traces is also a required standalone custom mod. Its shards live under `world/data/traces/<namespace>/<dimension>/`, so Prestige archives and restores them with the owning world. The V3 wrapper fixture proves legacy-lineage rejection, exact archive verification, old-world Traces inclusion, successor isolation, distinct random retry seeds, console forwarding, interruption recovery, idempotent lineage award, and automatic rollback after three bad health reports. Legacy server-root `data/traces` is deliberately neither imported nor deleted.
