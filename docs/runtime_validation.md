...
Server bundles make `run.sh` the prestige-supervised default entrypoint. The permission-level-4 Forge escape-hatch flow is `/prestige stage <biome>`, followed by `/prestige commit <world-name>`; `/prestige cancel` removes a staged request. These commands use the same service transitions and save finalization as the World Condenser UI, bypassing only its physical multiblock requirement. The supervisor alone consumes a committed request after Minecraft has finalized and released the save. `run-forge.sh` is the explicit raw Forge diagnostic bypass and never performs regeneration.
