...
- Realistic Ores already provides deposit blocks and crushed deposit items. This supports the intended chain `deposit -> crushed/preprocessed form -> molten/chemical outputs` without inventing custom intermediates for the starter subset.
...
- Create/PNCR already accepts ore tags and Chemlib acids. It can be extended from vanilla ore tags to Realistic Ores deposits after the deposit catalogue exists.
- Alchemistry types appear in runtime data and remain useful as recipe semantics for mod compatibility. The design direction is to replace player-facing Alchemistry machines with Create/PNCR parity, not to ignore Alchemistry-derived decomposition routes.
...
- Create-family additions should be audited as the replacement surface for player-facing chemistry/synthesis previously associated with Alchemistry, while Create/PNCR should cover dissolver-style parity where appropriate.
