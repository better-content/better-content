...
The headful lane remains failed and blocks a playable-stability claim. `/tmp/btm-vs-client-expanded-fallback` booted and joined a real Forge client with DH enabled, rendered the placed VS-family components, and opened a visible Eureka helm Assemble control, but both GUI activation and a targeted VS ship-assembler fallback left the server reporting zero ships. The run classified `ship_assembly_failure`; mount, movement, reconnect, save/reload, and two-client sync were therefore not reached. It also captured a blocking Shoulder Surfing/VS incompatibility warning and recoverable DH `ClientBlockStateColorCache` null-parent failures during client world load. Keep broader pack validation gated until this client repro is understood or explicitly accepted.
