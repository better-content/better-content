...
- Dimension Font placement is deployed from the reobfuscated chunk-clipped structure writer so structure callbacks do not write into far chunks under C2ME. Terrain reads no longer misuse `WorldGenLevel.ensureCanWrite` as a preflight check; that call emitted false `setBlock` errors at the world's minimum-height sentinel even when no write followed. Boxed placement remains authoritative because C2ME's callback `chunkPos` can differ from the active structure slice; the Forge worldgen GameTests must locate a real generated font and validate its complete altar before deployment. Latent chemical clouds are non-targetable hand/raytrace volumes and render as sparse non-occluding particles instead of depth-writing block models.
- `bcfixes` extends Forbidden & Arcanus' otherwise unconditional Lost Soul spawn predicate: natural placement must be underground without sky visibility and cannot use a grass block as its floor.
