...
[[ "$JAVA_MAJOR" == 17 ]] || die "Prestige requires Java 17, found ${JAVA_MAJOR:-unknown}"
shopt -s nullglob
PRESTIGE_JARS=("$SCRIPT_DIR"/mods/prestige-*.jar)
shopt -u nullglob
[[ "${#PRESTIGE_JARS[@]}" -eq 1 && -f "${PRESTIGE_JARS[0]}" && ! -L "${PRESTIGE_JARS[0]}" ]] \
  || die "expected exactly one regular mods/prestige-*.jar"
PRESTIGE_JAR="${PRESTIGE_JARS[0]}"

if [[ "${1:-}" == verify-archive ]]; then
  [[ "$#" -eq 4 ]] || die "usage: prestige-server.sh verify-archive ARCHIVE LINEAGE_ID TRANSACTION_ID"
  exec java -cp "$PRESTIGE_JAR" com.bettercontent.prestige.PrestigeArchiveVerifier verify "$2" "$3" "$4"
fi
...
for directory in "$STATE_DIR" "$CONTROL_DIR" "$ARCHIVE_DIR" "$TRANSACTION_DIR"; do
  [[ -d "$directory" && ! -L "$directory" ]] || die "unsafe Prestige state directory: $directory"
done
...
ensure_lineage() {
  if [[ ! -e "$LINEAGE_FILE" ]]; then
    ...
    [[ ! -e "$LEGACY_LINEAGE_V1" && ! -e "$LEGACY_LINEAGE_V2" && ! -e "$LEGACY_LINEAGE_V3" && -z "$legacy_control" ]] \
      || die "legacy Prestige state is unsupported; remove .prestige before starting v4"
    local entropy
...
verify_archive_against_world() {
  ...
  java -cp "$PRESTIGE_JAR" com.bettercontent.prestige.PrestigeArchiveVerifier verify-against \
...
create_verified_archive() {
  local input="$1" final="$2" partial="${2%.zip}.partial.zip" manifest="$1/prestige-archive-manifest-v1.tsv" digest
  ...
  if [[ -e "$partial" ]]; then
    if [[ -f "$partial" && ! -L "$partial" ]] && java -cp "$PRESTIGE_JAR" \
        com.bettercontent.prestige.PrestigeArchiveVerifier verify-against \
...
finalize_success() {
  ...
  verify_archive_against_world "$FINAL_ARCHIVE" "$ARCHIVE_INPUT/prestige-archive-manifest-v1.tsv"
...
preflight_reset_capacity() {
  ...
  [[ "$(stat -c '%d' -- "$world")" == "$(stat -c '%d' -- "$ARCHIVE_INPUT")" ]] \
    || die "active world and Prestige transaction state must share a filesystem"
...
validate_world_binding() {
  local binding="$ARCHIVE_INPUT/world/data/prestige/reset-binding-v4.tsv"
