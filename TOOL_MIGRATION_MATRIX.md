...
Active repo tooling under `tools/` is Kotlin-first. Active `tools/` source files must not use `.py` or `.sh`; this is validated through `tools/bc internal validate-kotlin-tool-surface`.
...
| Runtime validation | `tools/bc test smoke` | Disposable single-world server/client smoke and optional changed-scope evidence capture |
