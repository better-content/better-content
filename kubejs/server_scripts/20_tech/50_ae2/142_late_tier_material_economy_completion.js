...
// AE2 addon part/full conversions remain explicit casing-gated manufacturing steps.
ServerEvents.recipes(function (event) {
    if (Item.exists('expatternprovider:ex_interface_part')) {
        event.remove({ output: 'expatternprovider:ex_interface_part' })
        event.shapeless('expatternprovider:ex_interface_part', [
            'expatternprovider:ex_interface',
            'kubejs:impossible_machine_casing'
        ]).id('kubejs:ae2/expatternprovider/ex_interface_part')
    }
    if (Item.exists('expatternprovider:ex_interface')) {
        event.remove({ output: 'expatternprovider:ex_interface' })
        global.bcFactoryCrafting(event, 'kubejs:ae2/expatternprovider/ex_interface', 'expatternprovider:ex_interface', 1, [
            'PC',
            'BZ'
        ], {
            P: '#ae2:interface',
            C: 'ae2:capacity_card',
            B: 'kubejs:impossible_machine_casing',
            Z: 'ae2:logic_processor'
        })
    }
    if (Item.exists('expatternprovider:oversize_interface_part')) {
        event.remove({ output: 'expatternprovider:oversize_interface_part' })
        event.shapeless('expatternprovider:oversize_interface_part', [
            'expatternprovider:oversize_interface',
            'kubejs:impossible_machine_casing'
        ]).id('kubejs:ae2/expatternprovider/oversize_interface_part')
    }
    ...
    for (var i = 0; i < diskOutputs.length; i++) {
        if (!Item.exists(diskOutputs[i])) continue
        event.replaceInput({ output: diskOutputs[i] }, 'minecraft:amethyst_shard', 'kubejs:sky_steel_sheet')
    }
})
