...
Current fresh custom-mod and bundle evidence from 2026-07-16: all 19 bundled custom mods passed clean source `test` and `assemble` tasks and were deployed only from each ForgeGradle project's `build/reobfJar/output.jar`. `~/.cache/bc/fresh-custom-mods-smoke` then passed a reset-runtime smoke and a follow-up runtime validation with no hard failures. The dedicated server reached a playable world, produced no tick-behind or hard log signatures, saved in 1558 ms, and completed spawn preparation in 59311 ms. The runtime Unearthed regression audit passed with underground and aboveground evidence; residual vanilla-rock ratios were approximately 0.0022% underground and 0.0015% aboveground. Fresh CurseForge/client and complete-server archives passed ZIP integrity checks, all 19 embedded custom-jar hashes matched the tested repository artifacts, the client archive contained the exact tracked `options.txt`, and the extracted server archive passed runtime-shape inspection.
